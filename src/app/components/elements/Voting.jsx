...
class Voting extends React.Component {
    ...
    render() {
        ...
        const voting_manabar = current_account
            ...
            : 0;
        const current_mana = parseInt(voting_manabar
        ...
        const elapsed = new Date() / 1000 - last_update_time;
        const maxMana = net_vesting_shares * 1000000;
        let currentMana =
            parseInt(current_mana) + (elapsed * maxMana) / BLURT_VOTING_MANA_REGENERATION_SECONDS;
        ...
        const size = JSON.stringify(operation).replace(/[\[\]\,\"]/g, '').length;
        ...
        const cashoutDelta = parseInt((Date.parse(cashoutTime) - new Date()) / 1000);
...
export default connect(
    ...
    (state, ownProps) => {
        ...
        const recentClaims = parseInt(state.global.getIn([
            'reward_fund',
            'recent_claims',
        ]));
        const rewardBalance = parseFloat(state.global.getIn([
            'reward_fund',
            'reward_balance',
        ]));
        ...
        const currentSupply = parseFloat(state.global.getIn([
            'props',
            'current_supply',
        ]));
        const vestedBlurt = parseFloat(state.global.getIn([
            'props',
            'total_vesting_fund_blurt',
        ]));
        const vote_power_reserve_rate = parseInt(state.global.getIn([
            'props',
            'vote_power_reserve_rate',
        ]));
